...
def validate_ifs_file(filename, quiet=False):
    ...
    while line_num < len(lines):
        ...
        if '{' not in line:
            errors.append(f"Line {line_num + 1}: Expected IFS entry: {original_line.rstrip()}")
            line_num += 1
            continue
        
        # Extract the IFS name by working backwards from the opening brace
        brace_pos = line.index('{')
        before_brace = line[:brace_pos]
        after_brace = line[brace_pos + 1:]
        
        # Strip any trailing parentheses surrounded text (3D specification)
        name = re.sub(r'\(3D\)\s*$', '', before_brace, flags=re.IGNORECASE)
        
        # Strip leading and trailing whitespace
        ifs_name = name.strip()
        
        # Check if the closing brace is on the same line (single-line IFS)
        if '}' in after_brace:
        ...
        # Multi-line IFS: content after opening brace is part of the IFS body
        line_num += 1
        ...
        while line_num < len(lines):
            ...
            if '{' in line:
                errors.append(f"Line {line_num + 1}: Unexpected opening brace inside IFS entry '{ifs_name}': {original_line.rstrip()}")
            
            line_num += 1
        
        if not found_closing_brace:
            errors.append(f"IFS '{ifs_name}' starting near line {line_num}: Missing closing brace")
