...
def validate_lsystem_file(filename, quiet=False):
    """Validate that an L-System file follows the correct syntax."""
    try:
    ...
    while line_num < len(lines):
        ...
        if '{' not in line:
            errors.append(f"Line {line_num + 1}: Expected L-System entry: {original_line.rstrip()}")
            line_num += 1
            continue
        
        # Extract the L-System name by working backwards from the opening brace
        brace_pos = line.index('{')
        before_brace = line[:brace_pos]
        after_brace = line[brace_pos + 1:]
        
        # Strip leading and trailing whitespace
        lsystem_name = before_brace.strip()
        
        # Check if the closing brace is on the same line (single-line L-System)
        if '}' in after_brace:
        ...
        # Multi-line L-System: content after opening brace is part of the L-System body
        line_num += 1
        ...
        while line_num < len(lines):
            ...
            if '{' in line:
                errors.append(f"Line {line_num + 1}: Unexpected opening brace inside L-System entry '{lsystem_name}': {original_line.rstrip()}")
            
            line_num += 1
        
        if not found_closing_brace:
            errors.append(f"L-System '{lsystem_name}' starting near line {line_num}: Missing closing brace")
...
def main():
    parser = argparse.ArgumentParser(description='Validate L-System file syntax')
    parser.add_argument('filenames', nargs='+', metavar='filename', help='L-System file(s) to validate')
    ...
    for filename in args.filenames:
        success = validate_lsystem_file(filename, args.quiet)
        if not success:
            all_success = False
