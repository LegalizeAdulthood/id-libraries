...
comment = {This formula file davwierd.frm for FRACTINT was created by David Walter [100013,2775] 1 Jan 96.  If you manage to use them commercially please share some of the income with me!!  The parameter file davweird.par calls some of them.
The formulae with names beginning Ex#.. are derived from equations from the examples  # from Numerical Analysis 3rd Ed.by Richard L Burden, section 2.3 Newton Raphson.
The notes in comments are the notes I made whilst experimenting - I tend to take notes like I do in my  (Lab÷  Books!!
}
davidsmask = {
c = pixel,
...
comment = {15 Dec try ex1ccorrected using secant formula - first time! The method in Burden's book demands two initial guesses, which are (?) used to estimate f'.  It seems more sensible to estimate f' by {f(z) - f(z - del)}/del. This has been done, and the method called secdav. 
