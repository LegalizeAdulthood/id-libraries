...
cr=real(pixel)
cq=imag(pixel)
z=(cr*real(p1)+flip(cq*imag(p1)))*whitesq+((cr*real(p2)+flip(cq*imag(p2)))+p3)*(whitesq==0)
...
cr=real(pixel)
cq=imag(pixel)
z=(fn1(cr)*real(p1)+flip(fn2(cq)*imag(p1)))*whitesq+((fn3(cr)*real(p2)+flip(fn4(cq)*imag(p2)))+p3)*(whitesq==0)
